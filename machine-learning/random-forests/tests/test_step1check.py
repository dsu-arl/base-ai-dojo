...
class TestStep1Check(BaseTestValidator):
    def test_success(self):
        script_content = 'X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=23)'
...
"""
        ...
        cases = [
            {
                ...
                'script_content': 'X_train, X_test, y_train = train_test_split(X, y, test_size=0.3, random_state=23)'
            },
            {
                'desc': '5 output variables',
                'script_content': 'X_train, X_test, y_train, y_test, test = train_test_split(X, y, test_size=0.3, random_state=23)'
            }
        ]
        for case in cases:
    ...
    def test_incorrect_args(self):
        cases = [
            {
                'desc': 'Incorrect X',
                'script_content': 'X_train, X_test, y_train, y_test = train_test_split([], y, test_size=0.3, random_state=23)'
            },
            {
                'desc': 'Incorrect y',
                'script_content': 'X_train, X_test, y_train, y_test = train_test_split(X, test_size=0.3, random_state=23)'
            }
        ]
        for case in cases:
            with self.subTest(case=case['desc']):
        ...
        cases = [
            {
                ...
                'script_content': 'X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.8, random_state=23)'
            },
            {
                'desc': 'Incorrect random state',
                'script_content': 'X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=24)'
            }
        ]
        for case in cases:
            with self.subTest(case=case['desc']):
