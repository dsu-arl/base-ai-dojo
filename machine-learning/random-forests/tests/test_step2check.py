...
"""
    ...
    def test_output_not_assigned(self):
        ...
        is_correct, msg = validator._step_2_check()
        self.assertFalse(is_correct)
        self.assertEqual(msg, 'RandomForestClassifier() should be assigned to a variable')

    def test_function_has_args(self):
        cases = [
            {
                'desc': 'Has args',
                'content': 'model = RandomForestClassifier(23)'
            },
            {
                'desc': 'Has kwargs',
        ...
        for case in cases:
            with self.subTest(case=case['desc']):
                ...
                is_correct, msg = validator._step_2_check()
                self.assertFalse(is_correct)
                self.assertEqual(msg, "You shouldn't be passing any parameters to RandomForestClassifier() for this challenge")
